...
using Lykke.Job.HistoryExportBuilder.Modules;
using Lykke.Logs;
    ...
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            try
            {
                ...
                {
                    options.DefaultLykkeConfiguration("v1", "HistoryExportBuilder API");
                });
                ...
                builder.RegisterModule(new JobModule(appSettings.CurrentValue.HistoryExportBuilderJob, appSettings.Nested(x => x.HistoryExportBuilderJob), Log));
                builder.RegisterModule(new ClientModule(appSettings.CurrentValue, Log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            try
            {
                ...
                app.UseLykkeMiddleware("HistoryExportBuilder", ex => new ErrorResponse {ErrorMessage = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, IReloadingManager<AppSettings> settings)
        {
            ...
            var dbLogConnectionStringManager = settings.Nested(x => x.HistoryExportBuilderJob.Db.LogsConnString);
            ...
            var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                AzureTableStorage<LogEntity>.Create(dbLogConnectionStringManager, "HistoryExportBuilderLog", consoleLogger),
                consoleLogger);
